...
class SamiDepartments{
	...
	function __construct(){
		...
		*/
		...
		add_action('user_new_form',  array(&$this, 'new_user_department_section'));
		...
		add_action('user_register', array(&$this, 'save_user_department') );
...
<?php
	...
	//create the select group for edit users page.
	function new_user_department_section() {
		...
		<tr>
			...
			if ( !empty( $terms ) ) {
				...
				?>
					<li>
					<input type="checkbox" name="department[]" id="user-group-<?php echo esc_attr( $term->slug ); ?>" value="<?php echo esc_attr( $term->slug ); ?>" /> 
					<label for="user-group-<?php echo esc_attr( $term->slug ); ?>"><?php echo $term->name; ?></label></li>
				<?php 
				}
				echo '</ul>';
...
$sami_department = new SamiDepartments;
