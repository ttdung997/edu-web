...
class SimpleUserOrdering_Plugin {
    ...
    function alter_user_search($qry) {
        ...
        $qry->query_where = " LEFT OUTER JOIN wp_sm_21254_01dcv_hust_usermeta AS wp_sm_21254_01dcv_hust_usermeta_order ON (wp_sm_21254_01dcv_hust_users.ID = wp_sm_21254_01dcv_hust_usermeta_order.user_id AND wp_sm_21254_01dcv_hust_usermeta_order.meta_key = 'menu_order') " . $qry->query_where;
        $qry->query_orderby = preg_replace('/ORDER BY (.*) (ASC|DESC)/',"ORDER BY CAST(wp_sm_21254_01dcv_hust_usermeta_order.meta_value AS UNSIGNED) ".$qry->get('order') ,$qry->query_orderby);
