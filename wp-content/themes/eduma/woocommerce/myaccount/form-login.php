	...
	<div class="col-2 register-area" id="woo-register-area">
		...
		<form method="post" class="register">
			...
			<?php if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) : ?>
				...
				<p class="form-row form-row-wide">
					<input type="text" class="input-text" name="username" id="reg_username" placeholder="<?php esc_html_e( 'Username', 'eduma' ); ?>" value="<?php if ( ! empty( $_POST['username'] ) ) {
						echo esc_attr( $_POST['username'] );
					} ?>" />
				</p>
			...
			<p class="form-row form-row-wide">
				<input type="email" class="input-text" name="email" id="reg_email" placeholder="<?php esc_html_e( 'Email address', 'eduma' ); ?>" value="<?php if ( ! empty( $_POST['email'] ) ) {
