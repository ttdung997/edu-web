...
class lshowcase_ordering {
	...
	public function wp() {
		if ( 0 === strpos( get_query_var('orderby'), 'menu_order' ) && isset($_GET['dd']) &&($_GET['dd']=='true' ) ) {	
			...
			wp_deregister_style( 'lshowcase-smallicons' );
		    wp_register_style( 'lshowcase-smallicons', plugins_url( '/css/font-awesome/css/font-awesome.min.css', __FILE__ ),array(),false,false);
			wp_enqueue_style( 'lshowcase-smallicons' );	
	...
	public function sort_by_order_link( $views ) {
		$class = ( get_query_var('orderby') == 'menu_order title' && isset($_GET['dd']) && $_GET['dd'] == 'true'  ) ? 'lshowcase-active-drag' : 'lshowcase-inactive-drag';
		...
		if ( 0 === strpos( get_query_var('orderby'), 'menu_order' ) && isset($_GET['dd']) && ($_GET['dd']=='true' ) ) {	
			$views['byorder'] = '<span class="' . $class . '">Drag and Drop Sorting Activated</span> <i class="icon-move"></i>  <i style="color:#888;">Order is automaticaly saved after each Drag&Drop</i> <a href ="'. $query_string_inactive .'" class="lshowcase-inactive-drag"> Disable Drag&Drop </a> ';
		}
		else {
...
$lshowcase_ordering = new lshowcase_ordering;
